...
2. [proxy_cache_min_uses](http://nginx.org/r/proxy_cache_min_uses?_ga=1.82886422.1235345339.1438303904) 设置了在 Nginx 缓存一个条目前，它必须被客户端请求的次数。当缓存不断被填满时，这项设置便十分有用，因为这确保了只有那些被经常访问的内容才会被添加到缓存中。该项默认值为1（*译者注：比如proxy_cache_min_uses设为10，对于一个新的请求来说，前面10次的访问均不会命中cache并且在第10次访问的时候才生成cache*）。
